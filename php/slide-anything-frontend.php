...
function slide_anything_shortcode($atts) {
	...
	if ($id == 0) {
		...
		if (($post_status != 'publish') || (count($metadata) == 0)) {
			...
			// http://wordpressdemo/wp-content/uploads/2020/06/file.mp4
			...
			for ($i = 1; $i <= $slide_data['num_slides']; $i++) {
				...
				// $slide_content = "<div class='row'><div class='col-sm-9 col-md-7 col-lg-5 offset-lg-1'>".$slide_content."</div></div>";
	...
	$output = "</div>".$output."<div class='entry-content container'>";
	return $output;
}
